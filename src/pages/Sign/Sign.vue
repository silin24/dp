...
import {SAVE_USERLIST,SAVE_CARDARR,SAVE_SCREENID} from '../../store/mutations-type'
import {reqDeployRound,reqHome,reqSignUserList,} from '../../api/index'
  ...
  export default {
    ...
    methods: {
      ...
      //签到用户列表
      async getSignUserList(screenid){
        let b =  await reqSignUserList(screenid)
        console.log(b);
        let {code,list} = b
        if (code == 1) {
          console.log(list);
          let relist =  list.map((item)=>{
              let num = Math.floor(Math.random()*(200- 300) + 300);
              item.avator =`https://picsum.photos/${num}`
              return item
            })
          this.userList = relist
        }
      }
    },
    computed:{
