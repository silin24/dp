    ...
    <div class="drawBody">
        ...
        <div class="right">
          <div v-for="(item,index) in drawRounds" :key="item.round">
            ...
            <div class="persons" v-if="(item.round==nowDrawRound) && winUser">
    ...
    <div class="drawFooter">
      ...
      <div class="roundBox" v-show="showRoundBox">
        <div class="perRound" v-for="(item,index) in roundArr" :key="index">
          ...
          <div class="card">
           <!-- <Select  v-model="item.cardName"/> -->
           <Select  v-model="item.cardId"/>
  ...
  export default {
    ...
    data() {
      return {
        ...
        persons:'',
        roundArr:[
          {round:1,persons:8,cardId:'', isChecked:false,isClear:false},
          {round:2,persons:10,cardId:'', isChecked:false,isClear:false},
          {round:3,persons:12,cardId:'', isChecked:false,isClear:false},
          {round:4,persons:16,cardId:'', isChecked:false,isClear:false}
        ],
        clearAllRound:false,
        //正在抽奖轮次
        nowDrawRound:1
    ...
    methods: {
      ...
      async deployRound(){
        ...
        if (this.deployRoundText === '配置轮次') {
          ...
          this.roundArr = this.roundArr.map((item)=>{
            item.isClear = false
            return item
          })
        } else {
          ...
          if (roundData.length>0) {
            let data =  await reqDeployRound(roundData)
            console.log(data);
          }
        }
      },
      ...
      async clearRound(){
        ...
        if (this.clearRoundText === '清空轮次' ) {
          ...
          if (this.clearAllRound === true) {
            ...
            this.drawRounds = this.drawRounds.filter((item)=>{
              return item.isClear != true
            })
          }

          let data = await reqClearRound()
          console.log(data);
        }
      },
      //开始抽奖
      async start(){
        ...
        if (this.satrtAndOver === '开始') {
          ...
          console.log(a);
          // if (this.winUser.length > 5) {
          //   this.winUser = []
          //   this.nowDrawRound = this.nowDrawRound + 1
          // }
      ...
      toAddRound(e){
        console.log(e);
        let aa = {
          round:5,persons:20,cardId:'',isChecked:true,isClear:false
        }
